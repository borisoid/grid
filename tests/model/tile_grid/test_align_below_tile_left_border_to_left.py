from grid.model import Cell, Tile, TileCoordsAsCorners, TileGrid


def test_1() -> None:
    g1 = TileGrid.from_(
        Tile.build(TileCoordsAsCorners(Cell(6, 0), Cell(10, 5)), handle=1),
        Tile.build(TileCoordsAsCorners(Cell(7, 6), Cell(10, 10)), handle=2),
        Tile.build(TileCoordsAsCorners(Cell(0, 0), Cell(5, 5)), handle=3),
        Tile.build(TileCoordsAsCorners(Cell(0, 6), Cell(6, 10)), handle=4),
    )
    g2 = TileGrid.from_(
        Tile.build(TileCoordsAsCorners(Cell(6, 0), Cell(10, 5)), handle=1),
        Tile.build(TileCoordsAsCorners(Cell(6, 6), Cell(10, 10)), handle=2),
        Tile.build(TileCoordsAsCorners(Cell(0, 0), Cell(5, 5)), handle=3),
        Tile.build(TileCoordsAsCorners(Cell(0, 6), Cell(5, 10)), handle=4),
    )
